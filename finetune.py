...
from dataset.dataset_GS import MolTestDatasetWrapper
...
class FineTune(object):
    ...
    def train(self):
        ...
        if self.config['model_type'] == 'gin':
            ...
            from models.GS_GIN import GINet
            model = GINet(self.config['dataset']['task'], **self.config["model"]).to(self.device)
            model = self._load_pre_trained_weights(model)
        ...
        for epoch_counter in range(self.config['epochs']):
            print(config['dataset']['target'], "=====Epoch {}".format(epoch_counter))
            print('Training...')
            for bn, data in enumerate(tqdm(train_loader, desc="Iteration", ncols=80)):
                optimizer.zero_grad()
    ...
    def _load_pre_trained_weights(self, model):
        try:
            ...
            state_dict = torch.load(os.path.join(checkpoints_folder, 'model_99.pth'), map_location=self.device)
...
if __name__ == "__main__":
    ...
    if config['task_name'] == 'BBBP':
        ...
        config['dataset']['data_path'] = 'data/bbbp/BBBP'
        target_list = ["p_np"]

    elif config['task_name'] == 'Tox21':
        config['dataset']['task'] = 'classification'
        config['dataset']['data_path'] = 'data/tox21/tox21'
        target_list = [
        ...
        config['dataset']['task'] = 'classification'
        config['dataset']['data_path'] = 'data/clintox/clintox'
        target_list = ['CT_TOX', 'FDA_APPROVED']

    elif config['task_name'] == 'HIV':
        config['dataset']['task'] = 'classification'
        config['dataset']['data_path'] = 'data/hiv/HIV'
        target_list = ["HIV_active"]

    elif config['task_name'] == 'BACE':
        config['dataset']['task'] = 'classification'
        config['dataset']['data_path'] = 'data/bace/bace'
        target_list = ["Class"]

    elif config['task_name'] == 'SIDER':
        config['dataset']['task'] = 'classification'
        config['dataset']['data_path'] = 'data/sider/sider'
        target_list = [
        ...
        config['dataset']['task'] = 'classification'
        config['dataset']['data_path'] = 'data/muv/muv'
        target_list = [
        ...
        config['dataset']['data_path'] = 'data/freesolv/freesolv'
        target_list = ["expt"]

    elif config["task_name"] == 'ESOL':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'data/esol/esol'
        target_list = ["measured log solubility in mols per litre"]

    elif config["task_name"] == 'Lipo':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'data/lipophilicity/Lipophilicity'
        target_list = ["exp"]

    elif config["task_name"] == 'qm7':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'data/qm7/qm7'
        target_list = ["u0_atom"]

    elif config["task_name"] == 'qm8':
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'data/qm8/qm8'
        target_list = [
        ...
        config['dataset']['task'] = 'regression'
        config['dataset']['data_path'] = 'data/qm9/qm9'
    ...
    print(config)
    results_list = []
    config['dataset']['seed'] = 14
    seed = config['dataset']['seed']
    for target in target_list:
        config['dataset']['target'] = target
        result = main(config)
        results_list.append([target, result, seed])
